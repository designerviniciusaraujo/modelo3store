...
function fnShowCEP(IDProd){
  if(FC$.TypeFrt==3 || FC$.TypeFrt==4){
    ...
    sCEP+="        <div class='FieldCEP FieldCEPNum'><input type='text' placeholder='CEP' id='idZip"+ IDProd +"' value='"+ sNumCEP +"' maxlength='9' onkeyup='this.value=this.value.replace(/[^\\d]+/g,\"\");if(this.value.length >= 8)document.getElementById(\"idCEPButton\").click()'></div>";
...
function fnShowGlobalSignin(){
  ...
  if(oImgGlobalSign){
    ...
    if(typeof FC$.FacebookSigninID!="undefined"){
      sImgs+="| Entrar com <img src='"+ FC$.PathImg +"facebooklogin.svg' class='FacebookSigninClass' data-loginsuccess='fnLoginShowUserName'>";
      bFacebookLogin=true;
    }
...
FCLib$.onReady(function(){
  if(FCLib$.GetID("overlay")){
    ...
    DontGoBanner:FC$.PathImg +"bannerpopupdontgo.jpg?cccfc=2", //Banner
